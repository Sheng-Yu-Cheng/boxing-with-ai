...
Run from project root:
    python .\patch_game_system_v2.py

Fixes:
  1. src/game/input_sources.py now supports VisionAgent(config: TrajectoryVisionConfig).
  2. src/game/panda_renderer.py constants become:
       SCENE_RESIZE=0.11, SCENE_POS_Y=-2.0, SCENE_HEADING=0.0,
...
INPUT_SOURCES = r'''from __future__ import annotations
...
def patch_panda_renderer(path: Path) -> None:
    if not path.exists():
    ...
    backup(path)
    replacements = {
        r"SCENE_RESIZE\s*=\s*[-+0-9.eE]+": "SCENE_RESIZE = 0.11",
        r"SCENE_POS_X\s*=\s*[-+0-9.eE]+": "SCENE_POS_X = 0.0",
        r"SCENE_POS_Y\s*=\s*[-+0-9.eE]+": "SCENE_POS_Y = -2.0",
        r"SCENE_POS_Z\s*=\s*[-+0-9.eE]+": "SCENE_POS_Z = 0.0",
        r"SCENE_HEADING\s*=\s*[-+0-9.eE]+": "SCENE_HEADING = 0.0",
        r"OPPONENT_POS_X\s*=\s*[-+0-9.eE]+": "OPPONENT_POS_X = 0.0",
        r"OPPONENT_POS_Y\s*=\s*[-+0-9.eE]+": "OPPONENT_POS_Y = 0.0",
        r"OPPONENT_POS_Z\s*=\s*[-+0-9.eE]+": "OPPONENT_POS_Z = 0.0",
        r"OPPONENT_SCALE\s*=\s*[-+0-9.eE]+": "OPPONENT_SCALE = 1.0",
        r"OPPONENT_HEADING\s*=\s*[-+0-9.eE]+": "OPPONENT_HEADING = 0.0",
    }
    for pattern, repl in replacements.items():
        text, n = re.subn(pattern, repl, text, count=1)
        if n == 0:
            print(f"[warn] constant not found: {pattern}")
...
def main() -> None:
    root = Path.cwd()
    input_sources = root / "src" / "game" / "input_sources.py"
    panda_renderer = root / "src" / "game" / "panda_renderer.py"

    input_sources.parent.mkdir(parents=True, exist_ok=True)
    backup(input_sources)
    input_sources.write_text(INPUT_SOURCES, encoding="utf-8")
    print(f"[patched] {input_sources}")

    patch_panda_renderer(panda_renderer)
    print("\nDone. Try:")
    print(r"python .\scripts\game\run_game.py --input fusion --pose-model .\models\pose_landmarker_lite.task --classifier .\models\punch_classifier.joblib --confidence-threshold 0.60")
